...
// forked from s23b : https://www.shadertoy.com/view/4st3WX
...
vec4 hypnotize() {
  ...
  f *= sin(length(uv)*25. - .1);
  
  uv.x *=.4;
  f = min(f,1. - smoothstep(.27,.28,length(uv * vec2(1.,sin(frameCount*0.6)*.9 + 1.5))));
...
void main() {
  ...
  //col -= 0.1 * (rand(gl_FragCoord.xy/3.0 + frameCount + 100.0));
  ...
  } else if (toggleCheck == 4.0) {
    gl_FragColor = hypnotize();
  }
